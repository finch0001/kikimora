...
object PredictionComponent {
  ...
  case class State(
                    ...
                    addr: (List[String],List[String], List[String])
                    )
  //case class PredictionProps(loggedIn: Boolean)
  val component = ReactComponentB[/*PredictionProps*/Unit]("Prediction Component")
    .initialState(State(1, "", Map.empty,(Nil,Nil,Nil)))
    .renderPS((scope, props, state) ⇒ {

    def updateTags(tags: (Map[String,String],String,(List[String],List[String],List[String]))): Callback = scope.modState(_.copy(tags=tags._1, id=tags._2, addr = tags._3,formId=2))
    ...
    <.div(
      ...
      else if (state.formId == 2) {TableComponent.component(TableProps((state.tags, state.id),state.addr, resultPrintLn))}
